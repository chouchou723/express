...
//  let name = req.params.name//.split('%').length===1?req.params.name.split('%')[0]:req.params.name.split('%')[0]+'路';
   let name = rlencode.parse(`idnum:req.params.name`, {charset: 'gbk'});
    console.log(name);
    let base1 = {
        Accept: '*/*',
...
//         .type("form")
   ...
   request.post('https://shanghaicity.openservice.kankanews.com/public/bus/get').type('form').set(base1).send(name).end((err, resp) => {
